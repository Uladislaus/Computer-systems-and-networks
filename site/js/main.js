(() => {
  ...
  const fragSrc = `
    ...
    void main() {
      ...
      // Point stars with independent slow twinkle (unique speed + phase per cell)
      vec2 starGrid = uv * vec2(u_res.x / 3.0, u_res.y / 3.0);
      vec2 starCell = floor(starGrid);
      vec2 starLocal = fract(starGrid) - 0.5;
      float starOn = step(0.992, hash(starCell));
      float starDot = starOn * smoothstep(0.045, 0.0, length(starLocal));
