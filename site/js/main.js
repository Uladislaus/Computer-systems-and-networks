(() => {
  ...
  const fragSrc = `
    ...
    vec3 paintAbyss(vec2 uv, float aspect, float dive, float stage) {
      ...
      // Progressive reveals: jellies near surface first, silver school much later
      float showOpen = smoothstep(0.0, 0.16, s);
      float showSnow = smoothstep(0.04, 0.22, s);
      float showJelly = smoothstep(0.0, 0.16, s) * (1.0 - smoothstep(0.55, 0.74, s));
      float showKrill = smoothstep(0.2, 0.4, s);
      float showKelp = smoothstep(0.34, 0.56, s);
      float showSilver = smoothstep(0.56, 0.78, s);
      float showFish = smoothstep(0.62, 0.82, s);
      float showVent = smoothstep(0.72, 0.95, s);
      ...
      // Krill cloud — dense warm orange speck pack
      if (showKrill > 0.01) {
        float tK = u_time * 0.38;
        float ping = 0.5 - 0.5 * cos(tK); // smooth turnarounds
      ...
      // Jellyfish — appear first, higher in the water column
      if (showJelly > 0.01) {
        for (int i = 0; i < 3; i++) {
          float fi = float(i);
          float jx = aspect * mix(0.22, 0.82, hash(vec2(fi, 1.3))) + sin(u_time * 0.22 + fi * 2.0) * 0.06;
          float jy = mix(0.55, 0.82, hash(vec2(fi, 2.4))) + sin(u_time * 0.35 + fi) * 0.03;
      ...
      // Silver school — separate soft capsules, cos ping-pong (no face-stretch, no blob glow)
      if (showSilver > 0.01) {
      ...
      // Nearer dark school — soft silhouettes, cos ping-pong, spaced
      if (showFish > 0.01) {
        ...
        for (int i = 0; i < 8; i++) {
          ...
          float ox = (hash(vec2(fi, 1.0)) - 0.5) * 0.28;
          float oy = (hash(vec2(fi, 2.0)) - 0.5) * 0.16;
          float fx = aspect * (0.2 + ping * 0.55) + ox + sin(phase) * 0.03;
          float fy = 0.48 + oy + sin(phase * 1.3 + fi) * 0.045;
          float dx = x - fx;
          float dy = y - fy;
          float fish = smoothstep(0.0045, 0.0, abs(dy)) * smoothstep(0.02, 0.0, abs(dx));
          fish *= smoothstep(0.024, 0.006, abs(dx) + abs(dy) * 2.8);
          col = mix(col, col * 0.2, fish * 0.65 * d * showFish);
        }
      }
