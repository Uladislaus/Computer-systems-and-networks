(() => {
  ...
  const fragSrc = `
    ...
    void main() {
      ...
      // Constellations: Virgo + Scorpius — brighter named stars
      float skyStars = (1.0 - dawnAmt) * smoothstep(crestApprox + 0.1, 0.75, uv.y);
      if (skyStars > 0.01) {
        vec2 p = vec2(uv.x * aspect, uv.y);

        // --- Дева (Virgo): Y-фигура, Spica ярче внизу ---
        vec2 vi0 = vec2(0.22, 0.86); // Vindemiatrix
        vec2 vi1 = vec2(0.30, 0.82);
        vec2 vi2 = vec2(0.36, 0.76); // Porrima
        vec2 vi3 = vec2(0.28, 0.70); // Zavijava (левая ветвь)
        vec2 vi4 = vec2(0.42, 0.72); // Auva
        vec2 vi5 = vec2(0.48, 0.66); // Heze
        vec2 vi6 = vec2(0.52, 0.58); // Spica
        float vi =
          constellStar(p, vi0, 0.0048, 0.5, 1.0) +
          constellStar(p, vi1, 0.0042, 1.1, 1.2) +
          constellStar(p, vi2, 0.0050, 1.7, 0.95) +
          constellStar(p, vi3, 0.0045, 2.3, 1.15) +
          constellStar(p, vi4, 0.0044, 2.9, 1.05) +
          constellStar(p, vi5, 0.0046, 3.5, 1.25) +
          constellStar(p, vi6, 0.0072, 4.1, 0.75); // Spica
        float viLines =
          constellLine(p, vi0, vi1) + constellLine(p, vi1, vi2) +
          constellLine(p, vi2, vi3) + constellLine(p, vi2, vi4) +
          constellLine(p, vi4, vi5) + constellLine(p, vi5, vi6);
        col += vec3(0.88, 0.93, 1.0) * vi * 1.3 * skyStars;
        col += vec3(0.95, 0.9, 0.82) * constellStar(p, vi6, 0.0072, 4.1, 0.75) * 0.45 * skyStars;
        col += vec3(0.55, 0.68, 0.88) * viLines * 0.2 * skyStars;
